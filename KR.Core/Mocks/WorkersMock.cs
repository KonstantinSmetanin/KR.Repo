...
{
    public class WorkersMock : IWorkers
    {
        ...
        public void EmptyDB()
        {
            File.WriteAllText("DB.json", string.Empty);
            _workers.RemoveRange(0, _workers.Count);
        }

        public void Empty() => _workers = null;
        ...
        public void DeleteFromDB(Worker w)
        {
            _workers.Remove(w);
            SaveDB();
        }
        ...
        public Dictionary<string, int> EducatinoDivision(List<Worker> list)
        {
            ...
            foreach(string type in edTypes)
                res.Add(type, (from t in list where t.Education == type select t).ToList().Count());
        ...
        public static void SerializationTest(List<object> input)
        {
            string res = JsonSerializer.Serialize(input);
